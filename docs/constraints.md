...
- 다른 액터(GameState 등)의 준비를 기다려야 하면 **매 틱 폴링하지 않는다.** 준비 시점을 알리는 이벤트(예: `UWorld::GameStateSetEvent`)를 구독하고, 구독 시점에 이미 준비됐을 수 있으므로 등록 직후 한 번 더 직접 확인한다. 구독은 `EndPlay`/레벨 전환(Travel) 시 해제한다. (Pawn/Controller의 생명주기 훅이 다른 액터의 준비 완료를 보장한다고 가정하지 않는다 — 예: `BeginPlayingState`는 Pawn 자신의 상태 진입 훅일 뿐, GameState/PlayerState 준비 완료 훅이 아니다.)
